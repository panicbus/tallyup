...
export interface StaffPortContractSetup {
  port: StaffPort;
  seedBusiness(): Promise<{ id: string }>;
  seedStaff(input: { businessId: string; authUserId: string; role?: StaffRole }): Promise<{ id: string }>;
}
...
export function runStaffPortContractTests<Fixtures extends { realDb?: unknown }>(
  ...
  describe('StaffPort contract', () => {
    ...
    test('a code can only ever be redeemed once', async ({ realDb }) => {
      ...
      const first = await port.redeemInvite({ code: invite.code, authUserId: crypto.randomUUID(), email: 'a@example.com' });
      const second = await port.redeemInvite({ code: invite.code, authUserId: crypto.randomUUID(), email: 'b@example.com' });
    ...
    test('redeeming fails for an identity that already has an active staff row anywhere', async ({ realDb }) => {
      ...
      const invite = await port.createInvite({ businessId: businessB.id, role: 'staff', createdBy: owner.id });

      const result = await port.redeemInvite({ code: invite.code, authUserId: existingAuthUserId, email: 'x@example.com' });

      expect(result).toEqual({ outcome: 'already_staff' });
    });
    ...
    test('reactivates a deactivated staff row at the same business rather than creating a second one', async ({
      ...
      const invite = await port.createInvite({ businessId: business.id, role: 'owner', createdBy: owner.id });
    ...
    test('listStaff only reports unredeemed, unexpired invites for that business', async ({ realDb }) => {
      ...
      await port.createInvite({ businessId: businessA.id, role: 'staff', createdBy: ownerA.id });
      const redeemedInvite = await port.createInvite({ businessId: businessA.id, role: 'staff', createdBy: ownerA.id });
      await port.redeemInvite({ code: redeemedInvite.code, authUserId: crypto.randomUUID(), email: 'x@example.com' });
      await port.createInvite({ businessId: businessB.id, role: 'staff', createdBy: ownerA.id });
